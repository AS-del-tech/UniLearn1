...
import {
  ChevronDown, LogOut, Trash2, User, Flame,
  Focus, Download, Upload, X,
} from 'lucide-react';
...
export default function Header({ focusMode, onToggleFocus }: HeaderProps) {
  ...
  useEffect(() => { setStreak(getStreakCount()); }, []);
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">Your smart study platform</p>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                title="Export / Import"
                className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-secondary transition-all"
              >
              ...
              <Link href="/admin"
                className="text-sm text-muted-foreground hover:text-foreground transition-colors px-3 py-1.5 rounded-lg hover:bg-secondary hidden sm:inline-flex">
                Admin
              </Link>
                  ...
                  <div className="relative" ref={menuRef}>
                    ...
                    {showMenu && (
                      <div className="absolute right-0 mt-2 w-52 bg-white rounded-2xl shadow-lg border border-slate-100 py-1.5 z-20">
                        ...
                        <button onClick={() => { setShowMenu(false); setShowDeleteConfirm(true); setDeleteError(''); }}
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-6 w-full max-w-sm">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => { setShowExport(false); setImportMsg(''); }}
                className="w-7 h-7 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center">
                <X className="w-4 h-4" />
              </button>
            </div>
            <p className="text-sm text-muted-foreground mb-5">
              Download your notes, flashcards, and study data as JSON, or restore from a previous export.
            </p>
            <div className="space-y-3">
              ...
              <button onClick={() => fileRef.current?.click()}
                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium bg-slate-100 border border-slate-200 text-foreground hover:bg-slate-200 transition-all">
            ...
            {importMsg && (
              <p className={`text-xs mt-3 px-3 py-2 rounded-xl border text-center ${
