...
import {
  ...
  BookOpen, Zap, HelpCircle, FileText, Settings, Save,
} from 'lucide-react';
...
export default function StudyBuddy() {
  ...
  useEffect(() => {
    function handleOpen(e: Event) {
      ...
      setOpen(true);
      if (text) {
        setTimeout(() => { setInput(text); inputRef.current?.focus(); }, 150);
      }
    }
    window.addEventListener('studybuddy:open', handleOpen);
    return () => window.removeEventListener('studybuddy:open', handleOpen);
  }, []);

  function getNotesContext(): string {
    try { return localStorage.getItem('unilearn-notes') ?? ''; }
    catch { return ''; }
  }
  ...
  function handleSaveFlashcards(content: string) {
  ...
  return (
    <>
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 shrink-0"
            ...
            <div className="flex items-center gap-1">
              {hasNotes && (
                <span className="text-white/60 text-xs bg-white/10 px-2 py-0.5 rounded-full hidden sm:inline">
                  Notes attached
                </span>
              )}
            ...
            <div className="shrink-0 bg-slate-50 border-b border-slate-100 px-4 py-3 space-y-3">
              <div>
                ...
                <input
                  ...
                  maxLength={24}
                  className="w-full text-sm bg-white border border-slate-200 rounded-xl px-3 py-1.5 outline-none focus:ring-2 focus:ring-indigo-200"
                />
              </div>
              <div>
              ...
              <button onClick={saveSettings}
                className="w-full py-1.5 rounded-xl text-xs font-medium text-white transition-all"
                style={{ background: 'linear-gradient(135deg, #4F46E5, #7C3AED)' }}>
                Save settings
              </button>
            </div>
                ...
                <div className="flex flex-col gap-1.5 max-w-[78%]">
                  ...
                  {msg.role === 'assistant' && parseFlashcardsFromText(msg.content).length >= 2 && (
                    <button
                      ...
                      className="self-start flex items-center gap-1.5 px-2.5 py-1 bg-violet-50 border border-violet-100 rounded-full text-xs font-medium text-violet-700 hover:bg-violet-100 transition-colors"
                    >
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {QUICK_ACTIONS.map(({ label, icon: Icon }) => (
                  <button key={label} onClick={() => sendMessage(label)}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 border border-indigo-100 rounded-full text-xs font-medium text-indigo-700 hover:bg-indigo-100 transition-colors">
      ...
      <div className="fixed bottom-5 right-5 z-50">
        {!open && (
          <span className="ping-slow absolute inset-0 rounded-full"
