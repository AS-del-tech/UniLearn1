...
export async function POST(req: NextRequest) {
  ...
  const { allowed } = checkRateLimit(`flashcards:${ip}`);
  ...
  try {
    const completion = await client.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `You are a study assistant. Generate 5 to 10 flashcards from the provided text.
Return ONLY valid JSON — an array of objects with "question" and "answer" fields.
